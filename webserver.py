...
# It will serve a website that u can post blog_posts on
...
# where the blog_posts will be kept, must be in key value pairs, will be added by curr obj length + 1
database = {
...
def api_requests(conn:socket, headers, body) :
    try:
        if(headers[0] == "GET"):
            get_blog_post(conn)
        elif(headers[0] == "POST" and "/blog_post" in headers[1]): #if theres /blog_post in the request we are creating a blog_post
            create_blog_post(conn,body)
        elif(headers[0] == "POST"): #if blog_post is not in path, its just a login POST request
            login(conn,body)
...
def get_blog_post(conn:socket):
    blog_posts = json.dumps(database) #sends it in format {"ID#": "msg1", "ID#": "msg2"} etc etc...
    header = build_header(blog_posts, '.txt')
    conn.sendall(header.encode() + blog_posts.encode())

    

def create_blog_post(conn, body):
    try:

        new_blog_post_id = len(database) + 1

        msg = json.loads(body) #load the msg it should be in json form
        blog_post = msg["input"] #get the blog_post
        username = msg["username"]
        database[new_blog_post_id] = username + " says " + blog_post #store it in database

        #build a response header
        status_code = 'HTTP/1.1 200 OK\r\n'
    except Exception:
        status_code = 'HTTP/1.1 500 COULD NOT ADD blog_post TO DATABASE\r\n'
...
def login(conn, body):
    ...
    status_code = 'HTTP/1.1 200 OK\r\n'
    name = "Server: BLOGBYJAMES SERVER\r\n"
